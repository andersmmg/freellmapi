...
/**
 * V10 (May 2026):
 * 1. Add multimodal flag for vision-capable models (used by the router to
 *    skip text-only models when the request contains images).
 * 2. Seed Ollama Cloud — first new platform since Z.ai/Zhipu in V7.
 *    Free plan: GPU-time-based quota (not per-token), 1 concurrent model,
 *    5h session caps, no card required.
 */
function migrateModelsV10(db: Database.Database) {
  // 1) Multimodal column (no-op if already exists)
  ...
  const multimodals: Array<[string, string]> = [
  ...
  for (const [p, m] of multimodals) setFlag.run(p, m);
...
/**
 * V11 (May 2026):
 * 1. Seed audio transcription models.
 * 2. Fix Cerebras qwen3-235b → qwen-3-235b-a22b-instruct-2507 model_id.
 * 3. Re-enable NVIDIA NIM (free again ~1,000 starter credits).
 * 4. Add Kilo Gateway, Pollinations, LLM7.io (anon-friendly platforms).
 * 5. Seed Groq compound models, Cerebras gpt-oss-120b + llama3.1-8b.
 */
function migrateModelsV11(db: Database.Database) {
  // 1) Audio transcription models
  {
    const insert = db.prepare(`
  ...
  // 4) Add catalog rows for new platforms + compound models
  const insert = db.prepare(`
    INSERT OR IGNORE INTO models (platform, model_id, display_name, intelligence_rank, speed_rank, size_label, rpm_limit, rpd_limit, tpm_limit, tpd_limit, monthly_token_budget, context_window)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
